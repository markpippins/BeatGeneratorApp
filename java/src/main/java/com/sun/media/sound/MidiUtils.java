...
public final class MidiUtils {
    ...
    static Logger logger = LoggerFactory.getLogger(MidiUtils.class.getCanonicalName());
    /**
    ...
    public static long microsecond2tick(Sequence seq, long micros, TempoCache cache) {
        ...
        if (micros > 0 && cacheCount > 0) {
            ...
            logger.info("microsecond2tick(" + (micros / 1000) + ") = " + tick + " ticks.");
     ...
     *         if no entries are found that follow after tick, track.size() is
     *         returned
     */
    ...
    public static final class TempoCache {
        ...
        public int getCurrTempoMPQ() {
            logger.info("getCurrTempoMPQ");
            return currTempo;
        }

        float getTempoMPQAt(long tick) {
            logger.info(String.format("getTempoMPQAt Tick: ", tick));
