...
public class StrikeMixerPanel extends JPanel {
    ...
    private void initializeUI() {
        ...
        headerPanel.add(resetPanel, BorderLayout.EAST);
        ...
        scrollPane.setPreferredSize(new Dimension(700, 350)); // Height < 400px
        scrollPane.setMinimumSize(new Dimension(700, 350)); // Height < 400px
        
        // Add components to main panel
        add(headerPanel, BorderLayout.NORTH);
        add(scrollPane, BorderLayout.CENTER);
        
        // Close button at the bottom (simple and centered)
    ...
    private JPanel createChannelPanel(int drumIndex, String drumName) {
        ...
        // Add listener to update value label AND apply changes immediately
        slider.addChangeListener(new ChangeListener() {
            ...
            public void stateChanged(ChangeEvent e) {
                ...
                // Apply changes immediately (not just when slider stops)
                sequencer.setVelocity(drumIndex, value);
                ...
                // Only log when slider stops changing (otherwise too noisy)
                if (!slider.getValueIsAdjusting()) {
                    logger.debug("Updated drum {} ({}) volume to {}", 
                              drumIndex, drumName, value);
                }
            }
        });
    ...
    private void resetVolumes() {
        for (int i = 0; i < DRUM_PAD_COUNT && i < volumeSliders.size(); i++) {
            // Set slider value which will trigger the change listener
            // to update both the UI and the sequencer
            volumeSliders.get(i).setValue(100);
        }
        
        logger.info("Reset all drum volumes to default");
    }
    ...
    public static void showDialog(JComponent parent, DrumSequencer sequencer) {
        ...
        dialog.setSize(new Dimension(850, 360));
