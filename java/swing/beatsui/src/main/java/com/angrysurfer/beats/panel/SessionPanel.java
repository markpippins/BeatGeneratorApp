...
public class SessionPanel extends JPanel implements IBusListener {
    ...
    private void setupComponents() {
        ...
        // Remove the divider location code for the tableSplitPane since it no longer
        // exists
    }
    ...
    public void onAction(Command action) {
        ...
        switch (action.getCommand()) {
            case Commands.PLAYER_SELECTED:
                if (action.getData() != null && action.getData() instanceof Player) {
                    handlePlayerSelected((Player) action.getData());
                } else {
                    logger.warn("SessionPanel: Received PLAYER_SELECTED with invalid data: {}", action.getData());
                }
                break;
