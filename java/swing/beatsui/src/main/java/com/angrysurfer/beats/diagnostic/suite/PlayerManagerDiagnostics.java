    ...
    public DiagnosticLogBuilder testPlayerManager() {
        ...
        try {
            ...
            for (Player player : playerCache.values()) {
                ...
                if (player.isDrumPlayer() && channel != Constants.MIDI_DRUM_CHANNEL) {
                    channelErrors.add("Drum player " + player.getId() + " using non-drum channel: " + channel);
                } else if (!player.isDrumPlayer() && channel == Constants.MIDI_DRUM_CHANNEL) {
            ...
            try {
                ...
                for (Player player : playerCache.values()) {
                    if (player.isDrumPlayer() && (player.getChannel() == null || player.getChannel() != Constants.MIDI_DRUM_CHANNEL)) {
