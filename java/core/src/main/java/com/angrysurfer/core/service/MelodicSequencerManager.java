...
public class MelodicSequencerManager {
    ...
    // // Use existing player
    // logger.info("Using existing player {} for sequencer {}",
    // existingPlayer.getId(), id);
    ...
    // PlayerManager.getInstance().savePlayerProperties(player);
    //
    // logger.info("Created new player {} for melodic sequencer {}", player.getId(),
    // id);
    // }
    //
    // if (player.getInstrument() != null) {
    // MidiDevice device =
    // DeviceManager.getInstance().getMidiDevice(player.getInstrument().getDeviceName());
    // if (device != null)
    ...
    // if (sequenceData.getSoundbankName() != null || sequenceData.getBankIndex() !=
    // null ||
