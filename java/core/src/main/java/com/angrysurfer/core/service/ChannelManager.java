...
public class ChannelManager {
    ...
    public synchronized int getNextAvailableMelodicChannel() {
        for (int i = 0; i < channelsInUse.length; i++) {
            // Skip drum channel (9)
            if (i == SequencerConstants.MIDI_DRUM_CHANNEL) continue;
    ...
    public synchronized int getChannelForSequencerIndex(Integer sequencerIndex) {
        ...
        if (channel >= 16) {
            ...
            channel = channel % 16;
            if (channel == SequencerConstants.MIDI_DRUM_CHANNEL) channel = 0; // Avoid drum channel 9
        }
    ...
    public synchronized void releaseChannel(int channel) {
        if (channel >= 0 && channel < channelsInUse.length && channel != SequencerConstants.MIDI_DRUM_CHANNEL) {
    ...
    public synchronized boolean reserveChannel(int channel) {
        if (channel >= 0 && channel < channelsInUse.length && channel != SequencerConstants.MIDI_DRUM_CHANNEL && !channelsInUse[channel]) {
            channelsInUse[channel] = true;
