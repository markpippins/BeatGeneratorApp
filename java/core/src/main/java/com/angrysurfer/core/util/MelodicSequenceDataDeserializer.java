...
public class MelodicSequenceDataDeserializer extends StdDeserializer<MelodicSequenceData> {
    ...
    public MelodicSequenceData deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException {
        ...
        if (node.has("swing")) data.setSwing(node.get("swing").asInt());

        if (node.has("patternLength")) data.setPatternLength(node.get("patternLength").asInt());
        if (node.has("direction"))
            data.setDirection(mapper.treeToValue(node.get("direction"), data.getDirection().getClass()));
        if (node.has("timingDivision"))
            data.setTimingDivision(mapper.treeToValue(node.get("timingDivision"), data.getTimingDivision().getClass()));
        ...
        if (node.has("followNoteSequencerId")) data.setFollowNoteSequencerId(node.get("followNoteSequencerId").asInt());
        if (node.has("followTiltSequencerId")) data.setFollowTiltSequencerId(node.get("followTiltSequencerId").asInt());
