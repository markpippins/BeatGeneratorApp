...
/**
 * Core sequencer engine that handles drum pattern sequencing and playback.
 * This class is responsible for managing patterns, timing, and triggering notes.
 */
@Getter
@Setter
public class DrumSequencer implements IBusListener {
    ...
    public void generatePattern(int density) {
        ...
        for (int drumIndex = 0; drumIndex < DRUM_PAD_COUNT; drumIndex++) {
            ...
            if (strikes[drumIndex] == null) continue;
            
            // Different probability for different drums
            int drumDensity = density;
            if (drumIndex == 0) drumDensity = Math.min(50, density); // Less kicks
            if (drumIndex == 1) drumDensity = Math.min(40, density); // Less snares
            if (drumIndex == 2) drumDensity = Math.min(70, density); // More hi-hats
